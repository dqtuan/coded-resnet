...
def main():
	...
	net_g_model_out_path = os.path.join(checkpoint_path, "netG_model_combined.pth")
	device = torch.device("cuda:0")
	net_g = define_G(6, 3, 8, 'batch', False, 'normal', 0.02, gpu_id=device)
	...
	criterionMSE = torch.nn.MSELoss(reduction='sum').to(device)
	for epoch in range(100):
		for batch_idx, (inputs, targets) in enumerate(trainloader):
			...
			# loss = criterion(out_b_hat, out_b)
			loss = criterionMSE(out_b_hat, out_b)
			loss.backward()
			optimizer_g.step()
			...
			print("===> Epoch[{}]({}/{}): Loss: {:.4f} Obj: {:.4f}".format(
				epoch, batch_idx, len(trainloader), loss.item()/bs, objective/bs))
			objs.append(objective/bs)
	...
	print('Overall objective: ', np.mean(objs))
