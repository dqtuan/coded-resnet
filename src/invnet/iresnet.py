...
class conv_iResNet(nn.Module):
    ...
    def print_stats(self, z, debug=False):
        if debug:
            data = z.view(z.shape[0], -1).cpu().detach().numpy()
            norms = np.linalg.norm(data, axis=1)
            # stats = [np.min(data), np.mean(data), np.max(data), np.quantile(data, 0.25), np.median(data), np.quantile(data, 0.75), np.quantile(norms, 0.25), np.median(norms), np.quantile(norms, 0.75)]
