...
def main():
    ...
    result_path = os.path.join(args.save_dir, 'npy')
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        z_b = out_bij[bs:, ...]
        z_ab = (z_a + z_b) / 2
        x_c = model.module.inverse(z_ab, max_iter=10)
        # sanity check
        # torchvision.utils.save_image(x_c.cpu(), os.path.join(args.save_dir, "inverse_{}.jpg".format(batch_idx)), int(bs**.5), normalize=True)
        _, z_c = model(x_c)
        z_b_hat = z_c * 2 - z_a
        ...
        torchvision.utils.save_image(real_b.cpu(), os.path.join(args.save_dir, "real_b.jpg"), int(bs**.5), normalize=True)
        torchvision.utils.save_image(x_b.cpu(), os.path.join(args.save_dir, "inverse_b.jpg"), int(bs**.5), normalize=True)
        torchvision.utils.save_image(x_b_hat.cpu(), os.path.join(args.save_dir, "inverse_b_hat.jpg"), int(bs**.5), normalize=True)
        torchvision.utils.save_image(x_b_noise.cpu(), os.path.join(args.save_dir, "inverse_b_noise.jpg"), int(bs**.5), normalize=True)

        out_b = model.module.classifier(z_b)
        out_b_hat = model.module.classifier(z_b_hat)
        _, y_b = torch.max(out_b, 1)
        ...
        print('Correct %: real {:.4f} reconstruct {:.4f} match {:.4f}', correct_b/bs, correct_b_hat/bs, match_b/bs)
        # Store data
        img1 = inputs[:bs, ...].detach().cpu().numpy()
        img2 = inputs[bs:, ...].detach().cpu().numpy()
        img = x_c.detach().cpu().numpy()
