...
parser.add_argument('--coeff', default=0.9, type=float,
...
parser.add_argument('--inj_pad', default=0, type=int,
					help='initial inj padding')
...
def main():
	...
	result_path = os.path.join(args.save_dir, 'fusion')
	try_make_dir(args.save_dir)
	try_make_dir(result_path)
	...
	if args.deterministic:
		trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch,
												  shuffle=True, num_workers=2, worker_init_fn=np.random.seed(1234), drop_last = True)
		testloader = torch.utils.data.DataLoader(testset, batch_size=args.batch,
												 shuffle=False, num_workers=2, worker_init_fn=np.random.seed(1234), drop_last = True)
	else:
		trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch, shuffle=True, num_workers=2, drop_last = True)
		testloader = torch.utils.data.DataLoader(testset, batch_size=args.batch, shuffle=False, num_workers=2, drop_last = True)
	...
	checkpoint_path = os.path.join('../results', 'fusion/checkpoint/cifar10')
	net_g_model_out_path = os.path.join(checkpoint_path, "netG_model_mse.pth")
	device = torch.device("cuda:0")
	net_g = define_G(32*2, 32, 8, 'batch', False, 'normal', 0.02, gpu_id=device)
	...
	for batch_idx, (inputs, targets) in enumerate(testloader):
		...
		input_b = inputs[bs:, ...]
		targets = targets[bs:, ...	]
		cinput = torch.cat((input_a, input_b), dim=1)
		# time measure
		start_time = time.time()
		input_c = net_g(cinput)
		time_g = time_g + time.time() - start_time

		out_a, z_a = model(input_a)
		out_b, z_b = model(input_b)
		# time f
		start_time = time.time()
		_, z_c = model(input_c)
		time_e = time_e + time.time() - start_time

		start_time = time.time()
		z_b_hat = z_c * 2 - z_a
		out_b_hat = model.module.classifier(z_b_hat)
		time_c = time_c + time.time() - start_time

		_, predicted_b = torch.max(out_b.data, 1)
		_, predicted_b_hat = torch.max(out_b_hat.data, 1)

		# batch error
		correct_b += predicted_b.eq(targets.data).sum().item()
		correct_b_hat += predicted_b_hat.eq(targets.data).sum().item()
		total += bs
		same_b += predicted_b_hat.eq(predicted_b.data).sum().item()

	# print out
	print('B Classification Accuracy: ', correct_b / total)
	print('B-hat Classification Accuracy: ', correct_b_hat / total)
	print('Same Count: ', same_b/total)
	print('Average time G: {:.4f}, E: {:.4f}, C: {:.4f} over {} batches of size {}'.format(time_g/count_batch, time_e/count_batch, time_c/count_batch, count_batch, args.batch))

if __name__ == '__main__':
	main()
	# data = np.load('', allow_pickle=True)
	# data = torch.Tensor(data)
	# torchvision.utils.save_image(ta, "../inverse_0.jpg", int(128**.5), normalize=True)
